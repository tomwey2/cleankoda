...
from cleankoda.core.services.agent_states_service import get_agent_state_by_id, update_agent_state
...
def save_plan_to_db(content: str) -> bool:
    ...
    agent_state = get_agent_state_by_id(user_id)
    if not agent_state:
        return False
    update_agent_state(user_id=user_id, issue_id=agent_state.issue_id, plan_content=content)
    return True
