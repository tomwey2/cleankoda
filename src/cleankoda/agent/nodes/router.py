...
from cleankoda.agent.services.message_processing import filter_messages_for_llm
from cleankoda.agent.services.prompts import load_prompt
from cleankoda.agent.state import AgentState
from cleankoda.core.types import IssueType, PlanState, SkillLevelType, WorkingState
...
def create_router_node(runtime: RuntimeSettings):
    ...
    async def router_node(state: AgentState) -> dict[str, str]:
        ...
        system_message = load_prompt("system_messages/systemprompt_router.md", state)
        human_message = load_prompt("human_messages/prompt_routing.md", state)
